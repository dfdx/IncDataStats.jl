...
function aggregate!{N<:Number}(agg::StatsAggregator,
                               key::Any, values::Vector{N})
...
haskey(agg::StatsAggregator, key::Any) = haskey(agg.groups, key)
keys(agg::StatsAggregator) = keys(agg.groups)
values(agg::StatsAggregator) = values(agg.groups)
getindex(agg::StatsAggregator, k::Any) = getindex(agg.groups, k)
